...
export default function NewShiftCompanyPage() {
  const router = useRouter()
  const [saving, setSaving] = useState(false)
  const [name, setName] = useState('')
  const [active, setActive] = useState(true)
  const [selectedShifts, setSelectedShifts] = useState<string[]>(['morning', 'afternoon', 'night'])
  const [selectedDays, setSelectedDays] = useState<number[]>([1, 2, 3, 4, 5])
  const [shiftTimes, setShiftTimes] = useState<ShiftTimes>(() => {
    const initial: ShiftTimes = {}
    SHIFT_OPTIONS.forEach(({ value, defaultStart, defaultEnd }) => {
      initial[value] = { start: defaultStart, end: defaultEnd }
    })
    return initial
  })
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-medium" style={{ color: '#1a1a1a' }}>Nová firma</h1>
      </div>
